-- ===========================================================================
--  LES SITES DEVIENNENT DES LIEUX — CHARGEMENT AUTANT QUE LIVRAISON
--  Réf. SPECIFICATIONS.md § 6.2
--
...
--
--  ⚠️ DDL ÉCRIT À LA MAIN — NE PAS RÉGÉNÉRER PAR `migrate diff`.
--
--     Le calcul d'écart proposait de SUPPRIMER `delivery_sites` et
--     `delivery_site_requirements` pour recréer `sites` et `site_requirements`.
--     Un renommage lui est indiscernable d'une suppression suivie d'une
--     création : il aurait effacé les lieux, leurs consignes d'accès, leurs
--     exigences — et les opérations qui les désignent.
--
--     Les tables sont donc RENOMMÉES. Rien n'est perdu, les clés étrangères
--     suivent d'elles-mêmes, et les contraintes sont réalignées pour que la
--     comparaison suivante ne croie pas qu'elles manquent.
-- ===========================================================================

CREATE TYPE "site_usage" AS ENUM ('LOADING', 'DELIVERY');

-- --- Renommage, sans perte --------------------------------------------------
ALTER TABLE "delivery_sites" RENAME TO "sites";
ALTER TABLE "delivery_site_requirements" RENAME TO "site_requirements";
ALTER TABLE "partner_sites" RENAME COLUMN "delivery_site_id" TO "site_id";
ALTER TABLE "carrier_tariffs" RENAME COLUMN "delivery_site_id" TO "destination_site_id";

ALTER TABLE "sites" RENAME CONSTRAINT "delivery_sites_pkey" TO "sites_pkey";
ALTER TABLE "site_requirements" RENAME CONSTRAINT "delivery_site_requirements_pkey" TO "site_requirements_pkey";
ALTER INDEX IF EXISTS "delivery_sites_code_key" RENAME TO "sites_code_key";
ALTER INDEX IF EXISTS "delivery_sites_is_active_city_idx" RENAME TO "sites_is_active_city_idx";
ALTER INDEX IF EXISTS "delivery_site_requirements_site_id_is_active_idx" RENAME TO "site_requirements_site_id_is_active_idx";
ALTER INDEX IF EXISTS "delivery_site_requirements_site_id_type_id_key" RENAME TO "site_requirements_site_id_type_id_key";
ALTER TABLE "site_requirements" RENAME CONSTRAINT "delivery_site_requirements_site_id_fkey" TO "site_requirements_site_id_fkey";
ALTER TABLE "site_requirements" RENAME CONSTRAINT "delivery_site_requirements_type_id_fkey" TO "site_requirements_type_id_fkey";
ALTER TABLE "partner_sites" RENAME CONSTRAINT "partner_sites_delivery_site_id_fkey" TO "partner_sites_site_id_fkey";
ALTER TABLE "carrier_tariffs" RENAME CONSTRAINT "carrier_tariffs_delivery_site_id_fkey" TO "carrier_tariffs_destination_site_id_fkey";

-- L'index d'unicité du rattachement porte l'ancien nom de colonne.
DROP INDEX IF EXISTS "uq_partner_site_lieu";

-- --- Ce qui est réellement NOUVEAU ------------------------------------------
ALTER TABLE "sites" ADD COLUMN "usages" "site_usage"[];
ALTER TABLE "carrier_tariffs" ADD COLUMN "origin_site_id" UUID;
ALTER TABLE "operations" ADD COLUMN "origin_site_id" UUID;

ALTER TABLE "operations" ADD CONSTRAINT "operations_origin_site_id_fkey"
  FOREIGN KEY ("origin_site_id") REFERENCES "sites"("id") ON DELETE SET NULL ON UPDATE CASCADE;
ALTER TABLE "carrier_tariffs" ADD CONSTRAINT "carrier_tariffs_origin_site_id_fkey"
  FOREIGN KEY ("origin_site_id") REFERENCES "sites"("id") ON DELETE CASCADE ON UPDATE CASCADE;
...
RETURNS int AS $$
  SELECT COALESCE(NULLIF(value, '')::int, 60)
    FROM system_settings WHERE key = 'DOC_EXPIRY_ALERT_DAYS';
$$ LANGUAGE sql STABLE;
