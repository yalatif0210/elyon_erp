...
-- ===========================================================================
--  MIGRATION DE DONNÉES — SCISSION D'UN RELEVÉ EN DEUX (§ 25/08/2026)
--
--  Le DDL généré par Prisma ferait tout en une seule ALTER TABLE, colonnes
--  NOT NULL sans défaut comprises : il échouerait dès la première ligne déjà
--  présente. Étalé ici en étapes : colonnes nouvelles NULLABLES → verrous
--  existants levés → scission ligne par ligne → colonnes rendues NOT NULL →
--  anciennes colonnes déposées.
-- ===========================================================================

-- AlterTable : nouvelles colonnes, nullable le temps de scinder les données.
ALTER TABLE "measurement_records"
  ADD COLUMN "phase" "operation_phase",
  ADD COLUMN "vcf" DECIMAL(9,6),
  ADD COLUMN "volume_15" DECIMAL(18,6),
  ADD COLUMN "paired_loaded_volume_15" DECIMAL(18,6),
  ADD COLUMN "paired_measurement_id" UUID;

-- Verrous existants qui empêcheraient la scission : un seul relevé
-- authoritatif PAR OPÉRATION (l'ancien index, remplacé plus bas par le SQL
-- métier injecté sur (operation_id, phase)), et des volumes chargé/livré
-- NOT NULL (l'ancien schéma à deux bouts).
DROP INDEX IF EXISTS uq_measurement_single_authoritative;
ALTER TABLE "measurement_records"
  ALTER COLUMN "ullage_variance_pct" DROP NOT NULL,
  ALTER COLUMN "loaded_volume_15" DROP NOT NULL,
  ALTER COLUMN "discharged_volume_15" DROP NOT NULL;

-- Scission ligne par ligne : chaque relevé existant portait le chargé ET le
-- livré ; il devient un relevé CHARGEMENT et un relevé DECHARGEMENT,
-- rapprochés via paired_measurement_id / paired_loaded_volume_15 — exactement
-- le snapshot que posera désormais le service (reconcilierUllage).
DO $$
DECLARE
  r RECORD;
  v_chg_id uuid;
  v_dec_id uuid;
BEGIN
  FOR r IN SELECT * FROM measurement_records LOOP
    v_chg_id := gen_random_uuid();
    v_dec_id := gen_random_uuid();

    INSERT INTO measurement_records (
      id, reference, operation_id, phase, source, is_authoritative, inspector_id,
      measurement_date, observed_volume, observed_temp_c, vcf, volume_15, uom,
      measured_density_15, viscosity_cst, sulphur_pct, water_and_sediment_pct,
      flash_point_c, is_off_spec, entered_by_field_user_id, entered_by_user_id,
      validated_at, created_at, updated_at
    ) VALUES (
      v_chg_id, substr(r.reference, 1, 20) || '-CHG', r.operation_id, 'CHARGEMENT', r.source,
      r.is_authoritative, r.inspector_id, r.measurement_date,
      COALESCE(r.loaded_observed_volume, r.observed_volume, r.loaded_volume_15),
      COALESCE(r.loaded_temp_c, r.observed_temp_c, 15),
      COALESCE(r.loaded_vcf, 1),
      r.loaded_volume_15, r.uom, r.measured_density_15, r.viscosity_cst,
      r.sulphur_pct, r.water_and_sediment_pct, r.flash_point_c, false,
      r.entered_by_field_user_id, r.entered_by_user_id, r.validated_at,
    ...
    INSERT INTO measurement_records (
      ...
      measurement_date, observed_volume, observed_temp_c, vcf, volume_15, uom,
      measured_density_15, viscosity_cst, sulphur_pct, water_and_sediment_pct,
      flash_point_c, is_off_spec, paired_measurement_id, paired_loaded_volume_15,
      ullage_variance_pct, tolerance_id, alert_threshold_pct, critical_threshold_pct,
      ullage_alert_triggered, ullage_critical_triggered, ullage_ack_by_id,
      ullage_ack_at, ullage_ack_reason, ullage_derogation_id, hse_event_id,
      entered_by_field_user_id, entered_by_user_id, validated_at, created_at, updated_at
    ) VALUES (
      v_dec_id, substr(r.reference, 1, 20) || '-DEC', r.operation_id, 'DECHARGEMENT', r.source,
      r.is_authoritative, r.inspector_id, r.measurement_date,
      COALESCE(r.discharged_observed_volume, r.discharged_volume_15),
      COALESCE(r.discharged_temp_c, 15),
      COALESCE(r.discharged_vcf, 1),
      r.discharged_volume_15, r.uom, r.measured_density_15, r.viscosity_cst,
      r.sulphur_pct, r.water_and_sediment_pct, r.flash_point_c, r.is_off_spec,
      v_chg_id, r.loaded_volume_15,
      r.ullage_variance_pct, r.tolerance_id, r.alert_threshold_pct, r.critical_threshold_pct,
      r.ullage_alert_triggered, r.ullage_critical_triggered, r.ullage_ack_by_id,
      r.ullage_ack_at, r.ullage_ack_reason, r.ullage_derogation_id, r.hse_event_id,
      r.entered_by_field_user_id, r.entered_by_user_id, r.validated_at, r.created_at, r.updated_at
    );

    DELETE FROM measurement_records WHERE id = r.id;
  END LOOP;
END $$;

-- Colonnes désormais renseignées sur toutes les lignes : NOT NULL.
ALTER TABLE "measurement_records"
  ALTER COLUMN "phase" SET NOT NULL,
  ALTER COLUMN "vcf" SET NOT NULL,
  ALTER COLUMN "volume_15" SET NOT NULL,
  ALTER COLUMN "observed_volume" SET NOT NULL,
  ALTER COLUMN "observed_temp_c" SET NOT NULL;

-- Anciennes colonnes à deux bouts : déposées, remplacées par ce qui précède.
ALTER TABLE "measurement_records"
  DROP COLUMN "discharged_observed_volume",
  DROP COLUMN "discharged_temp_c",
  DROP COLUMN "discharged_vcf",
  DROP COLUMN "discharged_volume_15",
  DROP COLUMN "loaded_observed_volume",
  DROP COLUMN "loaded_temp_c",
  DROP COLUMN "loaded_vcf",
  DROP COLUMN "loaded_volume_15";

-- CreateIndex
CREATE UNIQUE INDEX "measurement_records_paired_measurement_id_key" ON "measurement_records"("paired_measurement_id");

-- CreateIndex
CREATE INDEX "measurement_records_operation_id_phase_idx" ON "measurement_records"("operation_id", "phase");

-- AddForeignKey
ALTER TABLE "measurement_records" ADD CONSTRAINT "measurement_records_paired_measurement_id_fkey" FOREIGN KEY ("paired_measurement_id") REFERENCES "measurement_records"("id") ON DELETE SET NULL ON UPDATE CASCADE;
