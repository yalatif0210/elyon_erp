...
BEGIN
  ...
  IF d.type::text <> p_type THEN
    RETURN format('la dérogation est de type %s, or %s est attendu', d.type, p_type);
  END IF;
  IF d.revoked_at IS NOT NULL OR d.status::text = 'REVOKED' THEN
    RETURN format('la dérogation %s a été RÉVOQUÉE le %s',
                  d.reference, to_char(COALESCE(d.revoked_at, d.updated_at), 'DD/MM/YYYY'));
  END IF;
  IF d.status::text <> 'ACTIVE' THEN
    RETURN format('la dérogation %s est au statut %s', d.reference, d.status);
  END IF;
  IF d.expires_at IS NOT NULL AND d.expires_at <= now() THEN
    RETURN format('la dérogation %s a EXPIRÉ le %s',
                  d.reference, to_char(d.expires_at, 'DD/MM/YYYY'));
  END IF;
  IF d.subject_id IS NOT NULL AND p_sujet IS NOT NULL AND d.subject_id <> p_sujet THEN
    RETURN format('la dérogation %s porte sur %s, pas sur %s',
                  d.reference, d.subject_id, p_sujet);
  END IF;
...
-- ---------------------------------------------------------------------------
CREATE OR REPLACE FUNCTION enforce_photo_requirement()
RETURNS TRIGGER AS $$
DECLARE
  regime text;
  intitule text;
...
BEGIN
  ...
  SELECT i.photo_policy::text, i.label INTO regime, intitule
    FROM hse_checklist_items i WHERE i.id = NEW.item_id;

  IF regime IS DISTINCT FROM 'REQUIRED' THEN
    RETURN NEW;
  END IF;

  SELECT count(*) INTO nb
    FROM operation_attachments a WHERE a.check_item_id = NEW.id;
...
CREATE TRIGGER trg_photo_requirement
  BEFORE INSERT OR UPDATE OF outcome ON operation_hse_check_items
  FOR EACH ROW EXECUTE FUNCTION enforce_photo_requirement();
...
-- ---------------------------------------------------------------------------
CREATE OR REPLACE VIEW v_controles_sans_photo AS
SELECT o.reference        AS operation,
 ...
 WHERE i.photo_policy = 'REQUIRED'
   AND ci.outcome::text <> 'PENDING'
   AND NOT EXISTS (
     SELECT 1 FROM operation_attachments a WHERE a.check_item_id = ci.id
   );

COMMENT ON VIEW v_controles_sans_photo IS
  'Points enregistrés sans la photo que leur régime exige (§ 7.1 bis). Le déclencheur l''empêche désormais ; cette vue montre l''antérieur, qu''aucune reprise ne peut réparer.';
