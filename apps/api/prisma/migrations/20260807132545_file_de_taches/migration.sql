...
UNION ALL
-- 9. Événement terrain refusé. L'agent l'a vu sur sa tablette, mais sa file
--    locale peut avoir disparu — et l'opération reste en suspens.
SELECT 'REFUS_TERRAIN', 'LOGISTICS_COORD', 'ANOMALIE',
       fr.operation,
       'Saisie terrain refusée (' || fr.agent || ') — ' || left(fr.rejection_reason, 120),
       'operations',
       fr.received_at
  FROM v_field_rejections fr
  JOIN operations o ON o.reference = fr.operation
