...
-- DropForeignKey
ALTER TABLE "fixed_cost_budgets" DROP CONSTRAINT "fixed_cost_budgets_author_id_fkey";

-- DropForeignKey
ALTER TABLE "fixed_cost_budgets" DROP CONSTRAINT "fixed_cost_budgets_currency_code_fkey";

-- DropForeignKey
ALTER TABLE "fixed_cost_budgets" DROP CONSTRAINT "fixed_cost_budgets_fiscal_year_id_fkey";

-- DropIndex
DROP INDEX "absorption_rates_cost_pool_id_fiscal_year_is_current_idx";

-- DropIndex
DROP INDEX "absorption_rates_cost_pool_id_fiscal_year_version_key";

-- AlterTable
ALTER TABLE "absorption_rates" DROP COLUMN "fiscal_year",
ADD COLUMN     "fiscal_year_id" UUID NOT NULL,
ALTER COLUMN "budgeted_base" SET DEFAULT 0;

-- AlterTable
ALTER TABLE "cost_pools" ADD COLUMN     "variability" "cost_variability" NOT NULL DEFAULT 'FIXED';

-- DropTable
DROP TABLE "fixed_cost_budgets";

-- CreateIndex
CREATE INDEX "absorption_rates_cost_pool_id_fiscal_year_id_is_current_idx" ON "absorption_rates"("cost_pool_id", "fiscal_year_id", "is_current");

-- CreateIndex
CREATE UNIQUE INDEX "absorption_rates_cost_pool_id_fiscal_year_id_version_key" ON "absorption_rates"("cost_pool_id", "fiscal_year_id", "version");

-- AddForeignKey
ALTER TABLE "absorption_rates" ADD CONSTRAINT "absorption_rates_fiscal_year_id_fkey" FOREIGN KEY ("fiscal_year_id") REFERENCES "fiscal_years"("id") ON DELETE RESTRICT ON UPDATE CASCADE;
...
UNION ALL
SELECT ex.year, ex.label, ex.statut,
       'Au moins un pool de charges FIXES',
       EXISTS (SELECT 1 FROM absorption_rates a
  ...
  LEFT JOIN LATERAL (
    SELECT sum(x.volume) AS volume FROM revise x
     WHERE x.fiscal_year_id = ar.fiscal_year_id
       ...
       AND (cardinality(cp.segments) = 0 OR x.segment = ANY (cp.segments))
  ) r ON true
  LEFT JOIN LATERAL (
    SELECT sum(y.volume) AS volume FROM realise y
     WHERE y.fiscal_year_id = ar.fiscal_year_id
       AND y.uom = ar.base_uom::text
       AND (cardinality(cp.segments) = 0 OR y.segment = ANY (cp.segments))
  ) v ON true
 WHERE ar.is_current
   AND cp.is_active;
