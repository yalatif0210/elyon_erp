...
BEGIN
  ...
  SELECT direct_floor, minimum_margin, currency_code, uom
    INTO t
    FROM resolve_margin_threshold(NEW.segment::text, NEW.product_id, CURRENT_DATE);

  IF t IS NULL THEN
    RAISE EXCEPTION
      'Aucun seuil de marge défini pour le segment % : le deal % ne peut pas être approuvé.',
      NEW.segment, NEW.reference
      USING ERRCODE = 'check_violation';
  END IF;

  -- Comparer deux grandeurs de devise ou d'unité différentes n'a pas de sens.
  -- On refuse plutôt que de convertir en silence : une conversion implicite
  -- rendrait le verrou incompréhensible le jour où il bloque.
  IF t.currency_code <> NEW.currency_code OR t.uom::text <> NEW.uom::text THEN
    RAISE EXCEPTION
      'Seuil de marge exprimé en %/% alors que le deal % est en %/%. Configurer un seuil dans cette devise et cette unité.',
      t.currency_code, t.uom, NEW.reference, NEW.currency_code, NEW.uom
      USING ERRCODE = 'check_violation';
  END IF;
