...
-- ⚠️ `ALTER TABLE cost_pools ALTER COLUMN variability SET DEFAULT 'FIXED'`
--    RETIRÉ ICI, VOLONTAIREMENT.
--
--    `migrate diff` le propose parce que `schema.prisma` porte encore
--    `@default(FIXED)` sur ce champ — un défaut que son propre commentaire dit
--    n'exister QUE pour la reprise des pools antérieurs à ce champ, retiré de
--    la base juste après pour qu'aucun pool créé ensuite n'hérite d'une
--    nature par omission (voir `model CostPool` dans schema.prisma). Cette
--    migration ne porte que le déplacement des réglages système par défaut :
--    elle ne doit pas réintroduire, en silence, un défaut que le schéma
--    documente lui-même comme un piège à éviter. Sujet distinct, à traiter
--    séparément si la dérive doit être corrigée à la racine.
