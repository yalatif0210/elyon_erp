-- AlterTable
ALTER TABLE "operation_assignments" ADD COLUMN     "carrier_tariff_id" UUID,
ADD COLUMN     "freight_variance_reason" VARCHAR(500),
ADD COLUMN     "tariff_amount" DECIMAL(18,4),
ADD COLUMN     "tariff_tolerance_pct" DECIMAL(6,3);

-- CreateTable
CREATE TABLE "carrier_tariffs" (
    "id" UUID NOT NULL,
    ...
    CONSTRAINT "carrier_tariffs_pkey" PRIMARY KEY ("id")
);

-- CreateIndex
CREATE INDEX "carrier_tariffs_carrier_id_is_active_effective_from_idx" ON "carrier_tariffs"("carrier_id", "is_active", "effective_from");

-- AddForeignKey
ALTER TABLE "carrier_tariffs" ADD CONSTRAINT "carrier_tariffs_carrier_id_fkey" FOREIGN KEY ("carrier_id") REFERENCES "partners"("id") ON DELETE CASCADE ON UPDATE CASCADE;

-- AddForeignKey
ALTER TABLE "carrier_tariffs" ADD CONSTRAINT "carrier_tariffs_product_id_fkey" FOREIGN KEY ("product_id") REFERENCES "products"("id") ON DELETE CASCADE ON UPDATE CASCADE;

-- AddForeignKey
ALTER TABLE "carrier_tariffs" ADD CONSTRAINT "carrier_tariffs_delivery_site_id_fkey" FOREIGN KEY ("delivery_site_id") REFERENCES "delivery_sites"("id") ON DELETE CASCADE ON UPDATE CASCADE;

-- AddForeignKey
ALTER TABLE "carrier_tariffs" ADD CONSTRAINT "carrier_tariffs_currency_code_fkey" FOREIGN KEY ("currency_code") REFERENCES "currencies"("code") ON DELETE RESTRICT ON UPDATE CASCADE;
...
-- ===========================================================================
--  SITES DE LIVRAISON PARTAGÉS ENTRE CLIENTS
...
BEGIN
  ...
  SELECT gen_random_uuid(),
   ...
   WHERE ps.delivery_site_id IS NULL
   ON CONFLICT (code) DO NOTHING;
...
CREATE UNIQUE INDEX uq_partner_site_lieu
  ON partner_sites (partner_id, delivery_site_id)
  WHERE delivery_site_id IS NOT NULL;
...
SELECT o.id                AS operation_id,
  ...
  JOIN delivery_sites ds ON ds.id = ps.delivery_site_id
  JOIN delivery_site_requirements r ON r.site_id = ds.id AND r.is_active
  JOIN site_requirement_types t ON t.id = r.type_id AND t.is_active;

COMMENT ON VIEW v_exigences_site IS
  'Exigences du site de livraison d''une opération (§ 6.2). Portées par le LIEU et non par le client : plusieurs clients exploitent le même site, et une consigne de sécurité recopiée est une consigne qui se périme d''un côté.';
...
SELECT ds.code, ds.name, ds.city,
  ...
  FROM delivery_sites ds
  JOIN partner_sites ps ON ps.delivery_site_id = ds.id
  JOIN partners p ON p.id = ps.partner_id
...
BEGIN
  ...
  SELECT string_agg(t.label || ' — ' || r.detail, ' · ' ORDER BY t.display_order)
    INTO manquantes
    FROM partner_sites ps
    JOIN delivery_site_requirements r ON r.site_id = ps.delivery_site_id AND r.is_active
    JOIN site_requirement_types t ON t.id = r.type_id AND t.is_active
   WHERE ps.id = NEW.destination_site_id
     AND r.is_blocking
     AND NOT EXISTS (
       SELECT 1 FROM operation_site_requirement_acks a
        WHERE a.operation_id = NEW.id AND a.requirement_id = r.id
     );

  IF manquantes IS NOT NULL AND manquantes <> '' THEN
    RAISE EXCEPTION
      'EXIGENCES DU SITE — l''opération % ne peut pas partir : %. Levez ces points et acquittez-les avant le chargement : s''y présenter sans eux, c''est repartir à vide avec un produit déjà payé.',
...
RETURNS TABLE (
  ...
  SELECT t.id, t.basis::text, t.amount, t.currency_code, t.uom::text, t.tolerance_pct
   ...
   WHERE t.carrier_id = p_carrier_id
     ...
     AND (t.delivery_site_id IS NULL OR t.delivery_site_id = p_delivery_site_id)
   ORDER BY (CASE WHEN t.delivery_site_id IS NOT NULL THEN 4 ELSE 0 END
...
BEGIN
  ...
  SELECT o.transport_mode::text AS mode, o.planned_volume, o.destination_site_id,
         d.product_id, ps.delivery_site_id
    INTO op
    FROM operations o
    JOIN deals d ON d.id = o.deal_id
  ...
  SELECT * INTO t FROM resolve_carrier_tariff(
    NEW.carrier_id, op.mode, op.product_id, op.delivery_site_id, CURRENT_DATE);
