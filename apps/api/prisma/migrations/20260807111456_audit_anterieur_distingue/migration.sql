...
--
--  Lue dans l'historique des migrations plutôt qu'écrite en dur : c'est la
--  date à laquelle le déclencheur a réellement été posé sur CETTE base, et
--  elle diffère d'un environnement à l'autre. Une date recopiée serait fausse
--  partout sauf ici.
-- ---------------------------------------------------------------------------
CREATE OR REPLACE FUNCTION date_verrou_preuve()
RETURNS timestamptz AS $$
BEGIN
  -- ⚠️ CORRIGÉ APRÈS COUP. La version d'origine interrogeait
  --    `_prisma_migrations` dans un corps SQL — donc résolu à la CRÉATION de
  --    la fonction. Cette table est interne à Prisma et n'existe pas dans la
  --    base de travail où les migrations sont rejouées : toute migration
  --    ULTÉRIEURE échouait, sur un message sans rapport avec elle.
  --
  --    Le corps est désormais en plpgsql — résolu à l'exécution — et la valeur
  --    vient des PARAMÈTRES. Le fichier historique est corrigé plutôt que
  --    laissé en l'état : il est rejoué à chaque migration, donc un fichier
  --    cassé casse tout l'avenir.
  RETURN COALESCE(
...
WHERE recorded_at IS NULL OR recorded_at >= date_verrou_preuve()
