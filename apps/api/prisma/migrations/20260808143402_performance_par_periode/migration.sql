...
RETURNS TABLE (debut date, fin date, origine text) AS $$
  SELECT f.starts_on,
         LEAST(f.ends_on, CURRENT_DATE),
         'exercice ' || f.year::text
    FROM fiscal_years f
   WHERE f.id = exercice_courant()
  UNION ALL
  SELECT (CURRENT_DATE - interval '12 months')::date,
         CURRENT_DATE,
         'douze derniers mois'
...
COMMENT ON FUNCTION periode_performance_defaut() IS
  'Fenêtre de lecture par défaut de la performance commerciale : l''exercice comptable courant, ou à défaut les douze derniers mois glissants. `origine` dit laquelle, pour que l''écran l''affiche.';
...
BEGIN
  SELECT p.debut, p.fin, p.origine INTO d, f, o FROM periode_performance_defaut() p;
  IF p_debut IS NOT NULL THEN d := p_debut; o := 'période choisie'; END IF;
  IF p_fin   IS NOT NULL THEN f := p_fin;   o := 'période choisie'; END IF;
...
COMMENT ON FUNCTION performance_commerciale(date, date) IS
...
CREATE VIEW v_performance_commerciale AS
  SELECT * FROM performance_commerciale(NULL, NULL);
...
BEGIN
  ...
  EXECUTE 'GRANT EXECUTE ON FUNCTION performance_commerciale(date, date) TO erp_app';
