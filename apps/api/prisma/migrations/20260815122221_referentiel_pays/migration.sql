...
-- AlterTable
ALTER TABLE "invoices" ADD COLUMN     "cancellation_reason" VARCHAR(1000),
...
-- Bootstrap du référentiel FERMÉ (arbitrage du 15/08) — AVANT les clés
-- étrangères ci-dessous : `partners`/`sites` portent déjà des lignes
-- ('CI', 'CH') que la contrainte validera immédiatement à sa création. Sans
-- ces lignes, la migration échoue sur les données existantes, pas sur le
-- jeu de démonstration à venir — `prisma/seed.ts` ne s'exécute qu'après.
...
-- AddForeignKey
ALTER TABLE "partners" ADD CONSTRAINT "partners_country_code_fkey" FOREIGN KEY ("country_code") REFERENCES "countries"("code") ON DELETE RESTRICT ON UPDATE CASCADE;

-- AddForeignKey
ALTER TABLE "sites" ADD CONSTRAINT "sites_country_code_fkey" FOREIGN KEY ("country_code") REFERENCES "countries"("code") ON DELETE RESTRICT ON UPDATE CASCADE;
